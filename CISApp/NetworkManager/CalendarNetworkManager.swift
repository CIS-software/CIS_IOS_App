...
struct CalendarNetworkManager {
    let router = Router<CISApi.Calendar>()
    ...
    func createTraining(trainCalendar: Schedule, completion: @escaping (_ msg: String?, _ error: String?) -> ()) {
        router.request(.createTraining(days: trainCalendar)) {data, response, error in
            
        }
    }
    
    func getTraining(completion: @escaping (_ trainDays: TrainDay?, _ error: String?) -> ()) {
        router.request(.trainingCalendar) { data, response, error in
            if error != nil {
            ...
            if let response = response as? HTTPURLResponse {
                ...
                case .success:
                    ...
                    do {
                        let apiResponse = try JSONDecoder().decode(TrainDay.self, from: responseData)
...
//        router.request( .login(email: email, password: passworcd)) { data, response, error in
//            if error != nil {
...
//protocol RequestComplition {
//    associatedtype T
//    func completeRequest(data: Data?, response: URLResponse?, _ error: Error?, completion: T)
//
//}
//
//class RequestResolver<ClosureType>: RequestComplition {
//    func completeRequest(data: Data?, response: URLResponse?, _ error: Error?, completion: ClosureType) {
//        <#code#>
//    }
//
//    typealias T = ClosureType
//
////    func completeRequest(data: Data?, response: URLResponse?, _ error: Error?) {
////        if error != nil {
////
////        }
////    }
